pipeline {
    ...
    stages{
        stage('Build Maven'){
            steps{
                checkout scmGit(branches: [[name: '*/main']], extensions: [], userRemoteConfigs: [[url: 'https://github.com/Iziik/jenkins-01']])
        ...
        stage('Deploy'){
            steps{
                sh 'docker run -d --name iziik-maven -p 8080:5000 "$IMAGE:$TAG"'
